...
def get_filename(taskpath, field,output_dir,zipstart,zipend,structure='std',zip_freq='month'):
	...
	if boinc[:6] in ['hadcm3','hadam4']:  #PW - added 'hadam4' here
...
#		if varname[:len(field_name)]==field_name:
		if int(stash_code)==var_stash_code:
			...
			# Check if this variable matches the required meaning period and cell method
			#if int(meaning_period)==int(var_meaning_period) and cell_method==var_cell_method:
			if ((int(meaning_period)==6 and var_meaning_period=='1.5') or (int(meaning_period)==int(var_meaning_period))) and cell_method==var_cell_method: #PW - added check for (int(meaning_period)==6 and var_meaning_period=='1.5'), since this corresponds to 6-hourly means being estimated as means over two time steps every 6 hours. I'm not sure why var_meaning_period=='1.5' in this case - I think it's set somewhere in the CPDN setup - perhaps it assumes a time step is 45 mins, as it might be in a low-res model?
...
def select_vars_field(nc_file,field_name,meaning_period,cell_method,vert_lev):
	...
	for varname,var in nc_file.variables.iteritems():
		if varname[:len(field_name)]==field_name:
			...
			# Check if this variable matches the required meaning period and cell method
			#if int(meaning_period)==int(var_meaning_period) and cell_method==var_cell_method:
			if ((int(meaning_period)==6 and var_meaning_period=='1.5') or (int(meaning_period)==int(var_meaning_period))) and cell_method==var_cell_method:  #PW - as for identical change in select_vars_stash()
				if vert_lev=='any': # Any level
					varnames.append(varname)
...
def process_netcdf(in_ncf,out_name,field,append,zip_freq='month'):
	...
	try:
		...
		if remap_data:
			...
			print(nc_in_var[:,:,lon_lat_idxs[1]:lon_lat_idxs[3], lon_lat_idxs[0]:lon_lat_idxs[2]])	
		...
		if append:
			...
			nc_out_var.setncatts({key:nc_in_var.__dict__[key] for key in nc_in_var.__dict__ if key!='_FillValue'})  #PW 28/5/19 I found that the code was failing at the line "dim_out_var[tlen:] = d[1][:]" below, where it tries to append a value to the time dimension, giving error "NetCDF: Not a valid data type or _FillValue type mismatch". Note the syntax seems to be that assigning to indices with values len(dim_out_var) or greater results in data being appended. This also happens where it attempts to append to nc_out_var below if the dim_out_var line is skipped. I found that excluding _FillValue from the attributes that are copied allows the script to run.
